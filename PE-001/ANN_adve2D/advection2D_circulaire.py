...
reseau = tf.keras.models.load_model("Advection_2D-structure-1.h5")
moyenne = 0# A compléter
sigma = 1 # A compléter
...
def predire_flux(i,j):
    ...
    data_input = np.concatenate((data_input[:,0:9],data_input[:,13:21]),axis=1)
    output=reseau.predict(data_input)
    return output[0] # [F1,F2,F3,F4] une liste
    ...
    if ischema == 1:
        ...
        for i in range(1,imax):
            ...
            for j in range(2,jmax-1+1):
                ...
                cf1num = flux[0] - flux[3]
                cf2num = flux[1] - flux[2]
        #utiliser F1-F4
        return cf1num, cf2num
#boucle

while np.log10(norm_res) > -5:
